...
@section('title','Straipsnio redagavimas')
...
@section('content')
    <div class="container">
        ...
        <div class="row justify-content-center ">
            <h2 class="text-center">Straipsnio redagavimas</h2>
                        ...
                        <form action="{{ route('admin.articles.update', $article->id) }}" method="POST"
                            ...
                            <div class="mb-3">
                                ...
                                <select id="category_id" class="form-select @error('category_id') is-invalid @enderror"
                                    ...
                                    @foreach($categories as $category)
                                        <option
                                            value="{{ $category->id }}" {{ $category->id == $article->category_id ? 'selected' : (old('category_id') == $category->id ? 'selected' : '') }}>{{ $category->title }}</option>
                                    @endforeach
                                </select>
                                @error('category_id')
                                <span class="alert-danger" role="alert">
                            ...
                            <div class="mb-3">
                                ...
                                <input id="link" type="text" class="form-control @error('link') is-invalid @enderror"
                                       name="link" value="{{ $article->link }}" autocomplete="link">
                                @error('link')
                                <span class="alert-danger" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
        ...
        <strong>{{ $message }}</strong>
                            ...
                            <div class="mb-3">
                                ...
                                <div id="content-editor" style="height: 300px;">{!! old('content', $article->content) !!}</div>
